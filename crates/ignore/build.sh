...
cargo build --target x86_64-apple-darwin --profile $CONFIGURATION
cargo build --target aarch64-apple-darwin --profile $CONFIGURATION
...
mkdir $LIBNAME/$LIBNAME'Rust.xcframework/macos-arm64_x86_64/Headers/libIgnore/'
mv $LIBNAME/$LIBNAME'Rust.xcframework/macos-arm64_x86_64/Headers/'*.* $LIBNAME/$LIBNAME'Rust.xcframework/macos-arm64_x86_64/Headers/libIgnore/'

# Add top-level module map so SPM can find the IgnoreRust module
cat > $LIBNAME/$LIBNAME'Rust.xcframework/macos-arm64_x86_64/Headers/module.modulemap' <<'MODULEMAP'
module IgnoreRust {
    header "libIgnore/SwiftBridgeCore.h"
    header "libIgnore/ignore.h"
    export *
}
MODULEMAP
